...
class EzsigntemplatedocumentCreateObjectV1Request {
    ...
    /**
     * Validates the JSON data with respect to <code>EzsigntemplatedocumentCreateObjectV1Request</code>.
     * @param {Object} data The plain JavaScript object bearing properties of interest.
     * @return {boolean} to indicate whether the JSON data is valid with respect to <code>EzsigntemplatedocumentCreateObjectV1Request</code>.
     */
    static validateJSON(data) {
        // check to make sure all required properties are present in the JSON string
        for (const property of EzsigntemplatedocumentCreateObjectV1Request.RequiredProperties) {
            if (!data[property]) {
                throw new Error("The required field `" + property + "` is not found in the JSON data: " + JSON.stringify(data));
            }
        }
        if (data['a_objEzsigntemplatedocument']) { // data not null
            // ensure the json data is an array
            if (!Array.isArray(data['a_objEzsigntemplatedocument'])) {
                throw new Error("Expected the field `a_objEzsigntemplatedocument` to be an array in the JSON data but got " + data['a_objEzsigntemplatedocument']);
            }
            // validate the optional field `a_objEzsigntemplatedocument` (array)
            for (const item of data['a_objEzsigntemplatedocument']) {
                EzsigntemplatedocumentRequestCompound.validateJsonObject(item);
            };
        }
...
EzsigntemplatedocumentCreateObjectV1Request.RequiredProperties = ["a_objEzsigntemplatedocument"];
