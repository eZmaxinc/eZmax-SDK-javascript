/**
 * eZmax API Definition (Full)
 * This API expose all the functionnalities for the eZmax and eZsign applications.
 *
 * The version of the OpenAPI document: 1.1.17
 * Contact: support-api@ezmax.ca
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 *
 */

import ApiClient from '../ApiClient';
import EzsignfolderRequestCompound from './EzsignfolderRequestCompound';

/**
 * The EzsignfolderEditObjectV1Request model module.
 * @module eZmaxAPI/model/EzsignfolderEditObjectV1Request
 * @version 1.1.17
 */
class EzsignfolderEditObjectV1Request {
    /**
     * Constructs a new <code>EzsignfolderEditObjectV1Request</code>.
     * Request for PUT /1/object/ezsignfolder/{pkiEzsignfolderID}
     * @alias module:eZmaxAPI/model/EzsignfolderEditObjectV1Request
     * @param objEzsignfolder {module:eZmaxAPI/model/EzsignfolderRequestCompound} 
     */
    constructor(objEzsignfolder) { 
        
        EzsignfolderEditObjectV1Request.initialize(this, objEzsignfolder);
    }

    /**
     * Initializes the fields of this object.
     * This method is used by the constructors of any subclasses, in order to implement multiple inheritance (mix-ins).
     * Only for internal use.
     */
    static initialize(obj, objEzsignfolder) { 
        obj['objEzsignfolder'] = objEzsignfolder;
    }

    /**
     * Constructs a <code>EzsignfolderEditObjectV1Request</code> from a plain JavaScript object, optionally creating a new instance.
     * Copies all relevant properties from <code>data</code> to <code>obj</code> if supplied or a new instance if not.
     * @param {Object} data The plain JavaScript object bearing properties of interest.
     * @param {module:eZmaxAPI/model/EzsignfolderEditObjectV1Request} obj Optional instance to populate.
     * @return {module:eZmaxAPI/model/EzsignfolderEditObjectV1Request} The populated <code>EzsignfolderEditObjectV1Request</code> instance.
     */
    static constructFromObject(data, obj) {
        if (data) {
            obj = obj || new EzsignfolderEditObjectV1Request();

            if (data.hasOwnProperty('objEzsignfolder')) {
                obj['objEzsignfolder'] = EzsignfolderRequestCompound.constructFromObject(data['objEzsignfolder']);
            }
        }
        return obj;
    }

    /**
     * Validates the JSON data with respect to <code>EzsignfolderEditObjectV1Request</code>.
     * @param {Object} data The plain JavaScript object bearing properties of interest.
     * @return {boolean} to indicate whether the JSON data is valid with respect to <code>EzsignfolderEditObjectV1Request</code>.
     */
    static validateJSON(data) {
        // check to make sure all required properties are present in the JSON string
        for (const property of EzsignfolderEditObjectV1Request.RequiredProperties) {
            if (!data[property]) {
                throw new Error("The required field `" + property + "` is not found in the JSON data: " + JSON.stringify(data));
            }
        }
        // validate the optional field `objEzsignfolder`
        if (data['objEzsignfolder']) { // data not null
          EzsignfolderRequestCompound.validateJSON(data['objEzsignfolder']);
        }

        return true;
    }

/**
     * @return {module:eZmaxAPI/model/EzsignfolderRequestCompound}
     */
    getObjEzsignfolder() {
        return this.objEzsignfolder;
    }

    /**
     * @param {module:eZmaxAPI/model/EzsignfolderRequestCompound} objEzsignfolder
     */
    setObjEzsignfolder(objEzsignfolder) {
        this['objEzsignfolder'] = objEzsignfolder;
    }

}

EzsignfolderEditObjectV1Request.RequiredProperties = ["objEzsignfolder"];

/**
 * @member {module:eZmaxAPI/model/EzsignfolderRequestCompound} objEzsignfolder
 */
EzsignfolderEditObjectV1Request.prototype['objEzsignfolder'] = undefined;






export default EzsignfolderEditObjectV1Request;

